...
Coincidences* Start(){
    ...
    C->SetAlphaBeta(0,0.0894,-17);
    C->SetAlphaBeta(1,0.0786,-52);
...
Coincidences* Co_sigma(){
    ...
    ofstream lthr("lower_thr_sigma.txt");
    if(!lthr){
        cout << "Failed to open lower_thr.txt" << endl;
        return 0x0;
    }
    ofstream win("window_sigma.txt");
    if(!win){
    ...
    for(int i=100; i <= 1000; i += 100) {
        ...
        for(unsigned int j = 0; j < n; j++){
            e = D->ch2e(2,D->GetData(j)->qlongs->at(2));
            sum += e;
            sum2 += e*e;
        }
        sigma = sqrt(sum2/n - (sum/n)*(sum/n));
        lthr << i << '\t' << sigma << endl;
    }
