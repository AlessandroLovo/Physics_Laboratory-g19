...
class Coincidences{
    public:
        ...
        //Constructor
        Coincidences(int channnel_number = 3){
        ...
        Coincidences(const char* filename, unsigned int channnel_number = 3){
            n_chan = channnel_number;
...
//             curr_index[0]<inbranch[0]->GetEntries() && curr_index[1]<inbranch[1]->GetEntries() && curr_index[2]<inbranch[2]->GetEntries()
        ...
        Coincidences* Filter(vector<int>* chan, vector<double>* e_min, vector<double>* e_max = NULL){

	    if ( e_max ==  NULL ) e_max = new vector<double>( e_min->size(), DBL_MAX );
