...
As can be seen from fig \ref{fig:FWHM:sim} the results are very similar to the analog version, and in both cases the best resolution is achieved filtering the energy spectra with a window around the Compton Edge ($\sim 300\si{\kilo\electronvolt}$ for \ce{^22Na} and $\sim 1\si{\mega\electronvolt}$ for \ce{^60Co}).
