...
The experimental setup consist of two collinear organic scintillators (DET 1 and DET 2), mounted on a sledge and facing each other. Between them there is a \ce{^22Na} source collimated by two lead bricks. The energy peaks of photons emitted by this source are shown in Table \ref{tab:gammavalue}.
...
During the last day, we disabled the digitizer FPGA and acquired two datasets of raw waveforms directly from the detectors. This allows us to simulate an a-posteriori software CFTD and FPGA, and compare it with the analogic one.
...
In fig. \ref{fig:waveform} a waveform sample is showed (yellow), as seen by oscilloscope and digitizer. The shape is, as expected, the same; in particular, both rise time (about $6\si{\nano\second}$) and falling time (about $20\si{\nano\second}$) are the same.  Moreover, the signal transformed from the CFTD is also showed (cyan); it is formed by an attenuated reply of the original signal summed to a inverted delayed one. While the analogic CFTD does the transformation through electronic components, the APosteriori one is computed computationally. Even in this case the two system return a similar signal; however, the shortness in time of the signal and the low time resolution of the digitizer (a sample each $1$ns) result in a rougher waveform.
...
\begin{itemize}[noitemsep]
    ...
    \item A comparison between analogic CFTD and a simulated a-posteriori ones, finding that the analogic one provide a better resolution. This result have different origins; the main one can be found in the sampling that the digitizer make, wasting precision in time and energy results.
\end{itemize}
\end{document}
