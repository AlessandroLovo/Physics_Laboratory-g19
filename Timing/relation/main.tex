...
The experimental setup consist of two collinear organic scintillators, mounted on a sledge, and a \ce{^22Na} source collimated between two lead bricks. The peaks for this source are in tab \ref{tab:gammavalue}.
...
\begin{figure}[H]
    \centering
    \resizebox{0.8\textwidth}{!}{\import{img/}{det1_calibr_final.tex}}
...
% \begin{figure}[H]
%   \centering
%   \subfloat[]{\includegraphics[width=0.45\textwidth]{img/sim_511.png}} \quad
%   \subfloat[]{\includegraphics[width=0.45\textwidth]{img/sim_1275.png}}
%   \caption{Relation between the resolution of the peak and the peak position}
%   \label{fig:sim}
% \end{figure}
...
\todo[inline]{How have the centroid been found?}
...
\todo[inline]{Which is the unit of $\mu$? Channels!}
...
\begin{figure}[H]
    \centering
    \begin{subfigure}[Analogic CFD]{
        \includegraphics[width=0.38\textwidth]{img/waveform_osc.jpg}
        \label{fig:waveform_osc}
    }\end{subfigure}
    \begin{subfigure}[APosteriori CFD]{
        \resizebox{0.58\textwidth}{!}{\import{img/}{waveform_sim.tex}}
        \label{fig:waveform_sim}
    }\end{subfigure}
    \caption{Waveforms (yellow) and monitor signals (cyan) for analogic and software CFD, set with the same parameters (fraction = $20\%$, delay = $\sim 5\si{\nano\second}$)}
    \label{fig:waveform}
\end{figure}

As can be seen from fig. \ref{fig:waveform}, the shape of waveform and signal is similar for the two system. The signal, however, is very short in time and the low time resolution of the digitizer (a sample each $1$ns) result in a rougher waveform.
...
\todo{Rifare grafico ma senza calibrare!}

Integrating the waveform (substracted of the baseline) over the domain, an energy spectra can be computed. As can be seen comparing fig. \ref{fig:aposteriori_energy} with fig. \ref{fig:det1:calibr}, some difference are clear:

\begin{itemize}[noitemsep]
    \item Only the $511\si{\kilo\electronvolt}$ peak is visible: the signal for the higher peak exceed the digitizer acceptance, and are therefore discarded;
    \item Due to the time discretization done before integration, the energy spectra is rougher; the resolution, therefore, get worse.
\end{itemize}
The combination of this two factors prevent the possibility to do a correct calibration of the graph.
