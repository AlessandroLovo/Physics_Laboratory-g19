...
\todo[inline]{Leggi la teoria, paragrafo 1.4. Spiega perché Richardson è sbagliata.}
In this case the error in the prediction of the current could be used as an estimate of the percentage of absorbed electrons: $\sim 96\%$\todo{Da dove è uscito 'sto numero?}.
...
By studying instead the V-I characteristics varying the Argon pressure one obtains the results in fig \ref{fig:constIf}. If one ignores the weird behavior of the $P = 8$ \si{\micro\bar} curve at high $V$\todo{Sono due punti palesemente sbagliati, reiettiamoli e basta secondo me}, it is visible that the main influence of the pressure on the curve is the value of the breakdown voltage, effect that can be better understood building the experimental Paschen curve (fig \ref{fig:DCP})\todo{Come l'abbiamo costruita?}.
...
\begin{figure}[H]
  ...
  \label{fig:DCP}
\end{figure}
\todo[inline]{No amici, questo fit è inguardabile, non prende nemmeno bene il minimo, direi di ammazzarlo}
...
\todo[inline]{Non abbiamo dato un risultato, un chiaro valore per questo minimo}
...
\begin{figure}[H]
	\centering
	\resizebox{0.7\textwidth}{!}{\import{img/Langmuir/}{dx_char.tex}}
...
\begin{figure}[H]
	\centering
	\resizebox{0.7\textwidth}{!}{\import{img/Langmuir/}{sx_char.tex}}
