...
\section{DC analysis of the plasma}
\subsection{Voltage-current characteristics of the discharge}
...
where $A = 7\cdot 10^5 \si{\ampere/\metre^2\kelvin^2}$, $\Phi =  4.55 \si{\volt}$ and $\Sigma$ is the surface of the filament \todo[inline]{compute $\Sigma$}

By using the previous found relation between the temperature and the filament current $T = k\cdot I_f^{\frac[f]{5}{7}}$ the expected plasma current can be computed.

\todo[inline]{Ma In Richardson non c'e' nessuna dipendenza dalla tensione di polarizzazione... inoltre in che regime di plasma siamo? Bagliore o arco?}

By studying instead the V-I characteristics varying the Argon pressure one obtains the results in fig \ref{fig:constIf}, where it is visible the influence of the pressure on the breakdown voltage, effect that can be better understood building the experimental Paschen curve.
...
\subsection{Paschen curve in DC condition}
...
As can be seen from fig \ref{fig:DCP} the curve is quite noisy so a fit with the theoretical formula of the Paschen curve would fail. On the other hand the Paschen's minimum is clear to be between 3 and 4 \si{\micro\bar}.
