...
As can be seen from fig \ref{fig:constP}, variations on filament current doesn't affect breakdown voltage, but changes the behavior of the plasma once it is ignited. This effect is mainly due to the thermoionic emission of the filament: the current emitted by the filament into the plasma can be modeled by Richardson law:

\begin{equation*}
  I = \Sigma A T^2 \exp(-\frac{e\Phi}{k_BT})
\end{equation*}

where for our tungsten filament $A = 7\cdot 10^5 \si{\ampere/\metre^2\kelvin^2}$, $\Phi =  4.55 \si{\volt}$ and $\Sigma = 2\pi r L \sim 79$ \si{\milli\metre^2} is the surface aera of the filament.

By using the previous found relation between the temperature and the filament current $T = k\cdot I_f^{\frac[f]{5}{7}}$, and assuming that the plasma current is the current emitted by the filament, one obtains:
\begin{gather*}
...
\todo[inline]{AAAAAAAAAA THE ERRORS}
\todo[inline]{REPLY: these are order of magnitude computations}

As can be seen from tab \ref{tab:Richardson} the prediction and the data differ by a factor $\sim 25$, that is almost the same for every value of the filament current, suggesting that the main error is either in the estimate of coefficient $J$ or in the assumption that the current emitted by the filament is the plasma current. The latter seems more reasonable since the plasma current varies with $V$ and can be explained by the predominance of electron absorption in the plasma at low $V$ and electron multiplication at high $V$.
In this case the error in the prediction of the current could be used as an estimate of the percentage of absorbed electrons: $\sim 96\%$.
...
In any case the model used for the filament is unable to explain the experimental data.
...
Another way to see this incompatibility is that from fig \ref{fig:DCP} it is clear that Paschen's minimum is between 3 and 4 \si{\micro\bar}, but this would imply that using the theoretical value for $A$,  $d \sim 2.4$ \si{\metre} , that is rather unphysical considering that the whole chamber is only 0.8 \si{\metre} long.
...
Then, the amplitude of the wave as function of the trend has been studied. Moving the probe perpendicularly to the grid and recording the amplitude of the wave, the trend in fig. \ref{fig:ionicsonic:expdecay} are found.
