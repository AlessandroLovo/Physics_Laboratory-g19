...
\todo{See fig 9. Should we filter also 1275keV peak to avoid events generated without the emission of the gamma? I think it's not necessary}
...
\todo{Is it coherent with the theory? Discuss in comment}
...
This spectrum has a too low statistic to allow any meaningful analysis, even if the acquisition runned for about 23 hours. The three-photons decay it's a very rare event and only with a very long acquisition time plus a powerful filter to discard two-photons noise some results can be obtained.
...
\begin{itemize}[noitemsep]
    ...
    \item The dead-time trend has been estimated, giving a direction in understanding if a measured frequency is or is not meaningful;
    \item An interesting value for the ratio between the two and three photons decay is obtained; Moreover, the result $R=(150\pm15)$ matches (in order of magnitude) the value expect from the theory ($R_{th}=373$)); we didn't expect a better precision in the estimation of $R$, due to the uncertainty on the correction factor, the geometry of the system and the decay model;
    ...
    \begin{itemize}[noitemsep]
        ...
        \begin{itemize}[noitemsep]
            \item Material effect: the mean lifetime of substances can be effected from the material in which they decays. In some studies concerning the decay of parapositronium in materials, values compatible with our result are obtained\footnote{See for example the second component in \emph{McGervey, John D. and Walters, Virginia F.: Correlation between Lifetime and Momentum for Positron Annihilations in Teflon. Phys. Rev. B 2(1970), \url{https://link.aps.org/doi/10.1103/PhysRevB.2.2421}}};
...
The experience can be however considered a success: despite the multiple problems with the HV module, we manage to complete a meaningful data acquisition and some interesting results.
\todo[inline]{To be reviewed}
\end{document}
